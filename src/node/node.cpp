...
Node *Node::neighbor_query2(int k, Heap<double> &pq,
                            std::vector<double> &querypoint,
                            std::vector<int> &taboo_list, int e_param) {
  Node *min_node{nullptr};
  int neighbors_explored{0};
  double min_dist{std::numeric_limits<double>::infinity()};
  for (Node *neighbor : m_edges_to) {
    ...
    } else if (distance < pq[0].m_value &&
               !pq.has_index(neighbor->get_m_id())) {
      pq.insert(Heapnode<double>(neighbor->get_m_id(), distance));
      pq.extractmax();
    }
...
void Node::add_neighbor(Node *node) { m_edges_to.push_back(node); }

void Node::set_value(std::vector<double> &x) { m_value = x; }

double Node::dist(Node &x) { return sqdist(m_value, x.m_value); }
