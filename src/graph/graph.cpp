...
Heap<double> Graph::query2(int k, std::vector<double> &querypoint,
                           std::mt19937 &mt, int e_param, int step) {
  ...
  for (int i = 0; i < step; i++) {
    Node *new_node =
        node->neighbor_query2(k, pq, querypoint, taboo_list, e_param);
    if (!new_node) {
      break;
    }
    node = new_node;
...
Graph::Graph(int k, std::vector<std::vector<double>> &data, bool verbose) {
  ...
  for (int i = 0; i < nmb_nodes; i++) {
    ...
    if (i % 100 == 0) {
      ...
      if (verbose)
        std::cout << "\tNode number " << i << " constructed at " << aika
                  << std::endl;
    }
  }
}
