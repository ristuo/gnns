...
std::vector<int> knn(int k, std::vector<double> &query_point, Graph &knn_graph,
                     int iter, std::mt19937 &mt, int e_param, int step) {
...
std::vector<int> list_linear_search(int k, std::vector<double> &querypoint,
                                    std::vector<std::vector<double>> &data,
  ...
  for (int i = 0; i < k; i++) {
    pq.insert(
        Heapnode<double>(indeces[i], sqdist(querypoint, data[indeces[i]])));
  }
...
std::vector<int>
parallel_linear_search(int k, std::vector<double> &querypoint,
                       std::vector<std::vector<double>> &data) {
  ...
  task_list[0] =
      std::async(std::launch::async, subset_linear_search, k,
                 std::ref(querypoint), std::ref(data), 0, chunk_size - 1);
  for (int i = 1; i < max_threads - 1; i++) {
    task_list[i] = std::async(std::launch::async, subset_linear_search, k,
                              std::ref(querypoint), std::ref(data),
                              i * chunk_size, (i + 1) * chunk_size - 1);
  }
  task_list[max_threads - 1] = std::async(
      std::launch::async, subset_linear_search, k, std::ref(querypoint),
      std::ref(data), data.size() - chunk_size, data.size() - 1);
