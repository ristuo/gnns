...
class Eval_res {
    public:
        ...
        Eval_res(double correct, double took_ms, double linear_took_ms) : m_correct(correct), m_took_ms(took_ms), m_linear_took_ms(linear_took_ms) {};
        Eval_res() {m_correct = 0.0; m_took_ms = 0.0; m_linear_took_ms = 0.0;};
...
Eval_res evaluate(T1 ann_function, T2 knn_function) {
    ...
    return Eval_res(sum/annres.m_result.size(), annres.m_took_ms, knnres.m_took_ms);
}
